...
public sealed class LocalSecurityService(IWebHostEnvironment environment, IDataProtectionProvider protection)
{
    ...
    {
        if (password.Length < 8) throw new InvalidOperationException("A senha do backup precisa ter pelo menos 8 caracteres."); var backupDirectory = Path.Combine(_storage, "backups"); Directory.CreateDirectory(backupDirectory); var zip = Path.Combine(Path.GetTempPath(), $"cortafe-{Guid.NewGuid():N}.zip");
        ...
        var output = Path.Combine(backupDirectory, $"cortafe-backup-{DateTime.Now:yyyyMMdd-HHmmss}.cfbackup"); await using var stream = File.Create(output); await stream.WriteAsync(Encoding.ASCII.GetBytes("CORTAFE1")); await stream.WriteAsync(salt); await stream.WriteAsync(nonce); await stream.WriteAsync(tag); await stream.WriteAsync(cipher); return output;
    }
}

...
public sealed class LongVideoEditorialAnalyzer(EditorialAnalyzer analyzer)
{
    ...
    public List<ClipCandidate> Analyze(List<TranscriptSegment> transcript, ProjectOptions options)
    {
        ...
        if (duration <= LongVideoThreshold) return analyzer.Analyze(transcript, options).OrderByDescending(clip => clip.Score).ToList();
        ...
        var selected = new List<(ClipCandidate Clip, int Chunk)>();
        ...
        {
            if (selected.Count(item => item.Chunk == candidate.Chunk) >= perChunk) continue;
            selected.Add(candidate); if (selected.Count == options.ClipCount) break;
        }
        foreach (var candidate in unique.Where(candidate => !selected.Contains(candidate)))
        {
            selected.Add(candidate); if (selected.Count == options.ClipCount) break;
        }
        return selected.Select(item => item.Clip).OrderByDescending(clip => clip.Score).ToList();
    }

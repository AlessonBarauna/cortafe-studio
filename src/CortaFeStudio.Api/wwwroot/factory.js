...
function factoryPipeline(batch){if(!batch.pipelineStages?.length)return'';const labels={acquire:'Aquisição',transcribe:'Transcrição',analyze:'Análise',generateCandidates:'Candidatos',refineHooks:'Hooks',calculateSocialScores:'Social Score',generateVariants:'Variantes',selectWinners:'Vencedores',analyzeAudio:'Áudio',analyzeVideo:'Vídeo',generateMetadata:'Copy',render:'Render',qualityGate:'Qualidade',schedule:'Agenda',publish:'Publicação'};return `<div class="factory-pipeline">${batch.pipelineStages.map(stage=>`<span class="pipeline-${stage.status}" title="${escapeHtml(stage.error||labels[stage.name]||stage.name)}"><i></i>${labels[stage.name]||stage.name}</span>`).join('')}</div>`}

...
(function(){
  ...
  function parse(command,clip,profile){
    const text=norm(command),changes={},reasons=[];
    if(has(text,'mais dinamico','mais dinâmica','mais dinâmica','dinamico','dinâmica','ritmo rapido','ritmo rápido')){changes.transitionStyle='dynamic';changes.playbackSpeed=profile==='louvor'?1:1.25;reasons.push('ritmo mais dinâmico');}
    ...
    if(has(text,'acompanhe o pregador','acompanhar pregador','pessoa em foco','locutor em foco','rosto em foco','centralize o pregador')){changes.layoutMode='fill';changes.cropFocus='speaker';reasons.push('locutor em foco');}
    ...
    if(has(text,'sem marca dagua','sem marca d agua','sem marca d’água','sem marca d’água','remova marca','tirar marca')){changes.watermarkEnabled=false;reasons.push('marca d’água removida');}
    ...
    return {changes,reasons,recognized:Object.keys(changes).length>0};
  }
  ...
  function showPlan(project,clip,panel){
    ...
    host.innerHTML=`<div class="ai-edit-plan-list">${Object.entries(result.changes).map(([key,value])=>`<span><small>${label(key)}</small><b>${displayValue(key,value)}</b></span>`).join('')}</div><p>${result.reasons.map(reason=>`✓ ${escapeHtml(reason)}`).join(' · ')}</p><button type="button" class="btn btn-gold btn-sm" data-ai-edit-apply>Aplicar estas alterações</button>`;
  ...
  async function apply(project,clip,panel,result,button){
    const old=button.textContent;button.disabled=true;button.textContent='Aplicando…';
    try{
      const payload={...result.changes};
      await api(`/api/projects/${project.id}/clips/${clip.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      Object.assign(clip,result.changes);clip.renderOutdated=!!clip.videoPath;
      ...
      toast(`✓ ${Object.keys(result.changes).length} alterações aplicadas ao corte`);
  ...
  function displayValue(key,value){if(typeof value==='boolean')return value?'Ligado':'Desligado';return ({dynamic:'Dinâmica',editorial:'Editorial',smooth:'Suave',split:'Duas pessoas',blur:'Fundo desfocado',fill:'Pessoa em foco',speaker:'Locutor',center:'Centro',vertical:'9:16',square:'1:1',landscape:'16:9',portrait:'4:5',clean:'Clean',impact:'Impacto',sermon:'Sermão',worship:'Louvor',podcast:'Podcast'})[String(value)]||String(value);}

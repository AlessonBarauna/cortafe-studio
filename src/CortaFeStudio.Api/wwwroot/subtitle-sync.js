...
document.addEventListener('click',event=>{if(!event.target.closest('[data-cc-mode="captions"]'))return;setTimeout(async()=>{const card=document.querySelector('.clip-card.active');if(!card)return;await ensureCleanSubtitlePreview(card);refreshSubtitlePreview(card);enableSubtitleDrag(card)},0)});

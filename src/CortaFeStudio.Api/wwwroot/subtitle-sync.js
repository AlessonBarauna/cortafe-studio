...
async function prepareSubtitleWorkspaceClip(project,clipId){const card=document.querySelector(`.clip-card[data-clip="${clipId}"]`),clip=project?.clips.find(item=>item.id===clipId);if(!card||!clip)return;const key=`${project.id}:${clipId}:${clip.lastPreviewFingerprint||''}`;if(subtitlePreviewPreparation.has(key))return subtitlePreviewPreparation.get(key);const task=(async()=>{const captions=document.querySelector('[data-cc-mode="captions"]'),before=document.querySelector('#preview video'),relative=before?.dataset.sourcePreview?Math.max(0,before.currentTime-clip.start):Math.max(0,before?.currentTime||0);if(captions)captions.disabled=true;await ensureCleanSubtitlePreview(card);if(clip.previewPath){const video=activateLiveSubtitlePreview(clip);if(video){const restore=()=>{video.currentTime=Math.min(Number.isFinite(video.duration)?video.duration:clip.end-clip.start,relative)};video.readyState>=1?restore():video.addEventListener('loadedmetadata',restore,{once:true});updateSubtitlePreview(video,clip)}}if(captions)captions.disabled=false})();subtitlePreviewPreparation.set(key,task);try{await task}finally{subtitlePreviewPreparation.delete(key)}}

...
function activateLiveSubtitlePreview(clip){const preview=document.querySelector('#preview');if(!preview)return;let video=preview.querySelector('video');if(!video?.dataset.sourcePreview||video.dataset.clipId!==clip.id){preview.innerHTML=`<video controls playsinline preload="metadata" data-source-preview="true" data-clip-id="${clip.id}" data-clip-start="${clip.start}" data-clip-end="${clip.end}" src="/api/projects/${current.id}/source#t=${clip.start},${clip.end}"></video><div class="subtitle-preview" aria-live="polite"></div><span class="subtitle-preview-mode">EDIÇÃO AO VIVO · FONTE SEM LEGENDAS GRAVADAS · ${time(clip.end-clip.start)}</span>`;video=preview.querySelector('video');video.addEventListener('loadedmetadata',()=>{video.currentTime=clip.start});video.addEventListener('timeupdate',()=>{const start=+video.dataset.clipStart,end=+video.dataset.clipEnd;if(video.currentTime<start-.05)video.currentTime=start;if(video.currentTime>=end){video.pause();video.currentTime=end}updateSubtitlePreview(video,clip)});}else if(!preview.querySelector('.subtitle-preview'))preview.insertAdjacentHTML('beforeend','<div class="subtitle-preview" aria-live="polite"></div>');return video;}

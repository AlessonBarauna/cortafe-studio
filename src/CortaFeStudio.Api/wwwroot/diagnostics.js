...
async function diagnosticsCenter() {
  ...
  try {
    ...
    host.insertAdjacentHTML('afterbegin', `<div class="studio-panel p-4 mb-4"><span class="eyebrow">MOTOR DE RENDERIZAÇÃO</span><h3 class="mt-2">${escapeHtml(encoder.name)}</h3><p class="mb-0 text-secondary">${encoder.hardwareAccelerated ? 'Aceleração por GPU testada e disponível.' : 'Fallback universal e confiável por CPU.'}</p></div>`);

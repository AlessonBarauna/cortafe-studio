(function () {
  ...
  function bindWorkspace(project) {
    ...
    document.querySelectorAll('[data-cc-lane]').forEach(lane=>lane.onclick=event=>{if(event.target.closest('[data-cc-track]'))return;const content=document.querySelector('.cc-timeline-content');content.dataset.selectedLane=lane.dataset.ccLane;document.querySelector(`[data-cc-mode="${lane.dataset.ccLane==='video'?'visual':'details'}"]`)?.click();toast(`Faixa de ${lane.dataset.ccLane==='video'?'vídeo':'áudio'} selecionada`)});
